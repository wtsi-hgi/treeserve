...
#ifndef SRC_INDEXED_MAP_HPP_
#define SRC_INDEXED_MAP_HPP_
...
// tree structure, there will be a single copy of each string in a static
// lookup table so that IndexedMap instances are of type <uint64_t, double>
// rather than <string double> with the key being the index in the lookup table
// of the actual key of the datum since there are static members, there will
// need to be a cpp file - this class can't be header only
...
class IndexedMap {
 public :
    ...
    ~IndexedMap() {
        auto it = datumMap.begin();
        for (; it != datumMap.end(); it++) {
            delete it->second;
        }
        datumMap.clear();
    }
    ...
    void combine(const IndexedMap& other) {
    ...
    void subtract(const IndexedMap& other) {
        // remove vector
